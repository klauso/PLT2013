...
`FCall`, in addition to the original `Call`, that allows any `Exp` in the head
position (the first argument of `FCall`) a function call.  Note that we have
also added another construct `Defun` that allows users to define their own
functions.  Take the following declarations as a starting point, implement an
...
/* The difference between the two is that for `Call` you can directly
   call the function bound to the name `f`, but for `FCall`, you need do
   something first. */
case class Call(f : Symbol, argExp : Exp) extends Exp
case class FCall(funExp : Exp, argExp : Exp) extends Exp

/* Evaluating `Defun` should not return a `Closure` as `Value` since `Closure`
   is not a `Value` in this language.  Think about what you should return. */
