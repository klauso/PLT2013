...
that wraps a function name into an `Exp`.  Then we introduce a new construct
`FCall` that allows any `Exp` in the head position (the first argument of
`FCall`) a function call.  The difference between `Call` and `FCall` when
evaluated is that you can directly call the function bound to `f` for the
former, while for the latter you need do something to `funExp` first.  We
also introduce another construct `Defun` that allows users to define their own
functions.  Note that evaluting `Defun` should not return a `Closure` as value
since `Closure` is not a `Value` in this language.  Think about what you should
return.  Take the following declarations as a starting point, implement an
environment-based evaluator for this language.
