...
very beginning of the course: lecture notes, comments, exercises all _discuss_
...
the three laws are _not_ the equal sign `=` in Scala, they belong to our
meta-meta-language, that is, English plus mathematics.  So this exercise
does _not_ requires Scala programming and testing, but do require Scala program
understanding and mathematical equational reasoning.

- The annonymous function appearing in the Associativity law is not quite valid
Scala per se due to the lack of parameter type.  But simply ignore this minor
issue.
...
1. Rephrase the three laws in terms the `do`-notation defined below:
   ...
   _Hint:_ Get closer, doesn't it look familiar?

2. Show that the `IdentityMonad`, the `ReaderMonad`, the `StateMonad` and the
`ContinuatioinMonad` (optional, recommended) all satisfy these laws.  You can
choose to work with the definitions in either lecture note [18-monads.scala](../../lecturenotes/18-monads.scala)
or [monad-library.scala](../../lecturenotes/19-monads-library.scala).
